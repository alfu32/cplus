...
`TreeSitterCPlusPrototypeTranspiler` composes parser-backed lowering for `@throws` declaration extraction, statement-oriented `@try`/`@catch`, direct function-body `defer`, explicitly resolved value/pointer/static method calls, and struct method extraction. It reparses between structural transformations, keeps a mapped output, adds the empty annotation macros, and fails closed on comptime, test, and C-plus import syntax. The CLI and production `CPlusTranspiler` do not use this prototype.
