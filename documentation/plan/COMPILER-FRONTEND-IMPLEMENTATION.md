...
- Latest verification: legacy-output golden and focused interpolation/AST tests pass with exact generated-C, source-map, and stable-node assertions. The shared legacy/Tree-sitter construct gate passes for eight representative standard-library files and the interpolation fixture. Import-namespace tests cover `stdlib:/`, `module:/`, `project:/`, extensionless resolution, dependency ordering, and root traversal rejection. All 90 Tree-sitter corpus cases pass, generated parser source matches regeneration, and `./gradlew test build --no-daemon` passes on Linux x86_64 / JDK 21. The parser JNI output path and workflow YAML were checked locally; other host-matrix jobs remain unexecuted. Scoped `git diff --check` is clean for compiler, parser, workflow, and architecture-document changes; unrelated staged planning files are preserved.
