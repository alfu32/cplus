...
- [x] Add an opt-in Tree-sitter AST allocation-intent slice for direct `alloc_*`/`calloc_*`/`realloc_*` variable initializers; expose symbols and identifier-mapped mismatches through the prototype result without changing the production analyzer.
- [ ] Port alias propagation, annotated parameter/return contracts, nested scopes, and call-site allocation diagnostics; keep the legacy path until parity gates pass.
...
- Phase 6 allocation-analysis verification: targeted JVM tests exercise direct AST-domain mismatch detection, identifier-exact source spans, and propagation into both prototype and `TranscodedSource` results. Full flow-sensitive parity is deliberately not claimed; the legacy analyzer remains the production path.
