...
- [x] Infer the resulting allocation provenance of assignment and comma expressions in value contexts (the assignment RHS and final comma operand respectively); this does not infer side effects or short-circuit/control-flow state.
- [ ] Extend allocation analysis to expression-aware call evaluation and branch-sensitive data flow; keep the legacy path until parity gates pass.
...
- Phase 8 groundwork verification: `CPlusParseJson` serializes the same normalized AST and diagnostics independently of editor platform; the schema is versioned and explicitly declares UTF-16 offsets. Tests cover escaping, Unicode-containing buffers, category serialization for C declarations/control flow and comptime interpolation, and Node JSON parsing when Node is available. The CLI `parse` command returns this JSON for valid and recovered malformed files and is included in the fat CLI runtime; a VS Code opt-in setting consumes parser diagnostics on save. IntelliJ/Vim and AST-backed symbols/completions remain open.
- Phase 7 source-map verification: the prototype emits `#line` directives from mapped origins; an integration test compiles an invalid lowered method with each locally available TCC/GCC/Clang and checks the original `.cp` filename and line. This exposed and fixed `MappedText.originAt` indexing past EOF: end-of-buffer and invalid offsets now return no origin. Remote platform/toolchain matrix verification remains open.
...
- Phase 5 semantic-index verification: tests cover struct/method and top-level function symbols, access and ownership/mutation metadata, explicit instance/static call resolution, inferred `self` receiver resolution inside methods, parenthesized address/dereference receiver typing and correct already-pointer argument lowering, chained field receiver lookup from declared struct field types, named alias chains, const-qualified aliases, pointer typedefs, anonymous struct typedef ownership and method resolution, typed parameters, nested local shadowing, distinct `@throws()` / `@throws(error)` conventions with error-out parameter placement, and catch code/binding extraction. Function-pointer typedefs retain callable signature metadata (base result, parameter names/base types and source spellings, variadic state, declarator span) plus the complete original typedef declaration, including pointer/qualifier spelling. Callback parameters recurse and two nested function-pointer return-composition layers are indexed; a C interoperability regression compiles/runs fixed, variadic, nested callback, and callback-return typedefs on host `cc`. More complex qualified declarator normalization, target compiler coverage, and comptime-scope semantics remain open.
- Phase 6/7 foundation verification: `TreeSitterCPlusPrototypeTranspiler` composes AST-bounded `@test` extraction, throws extraction/removal, statement-oriented checked-call/try-catch lowering, defer, receiver-call, and struct-method passes, reparsing between stages while retaining `MappedText` origins. Test extraction retains decoded names, mapped bodies/spans, and AST-indexed assertions; the existing harness compiles and executes extracted fixtures. AST defer handles nested control flow with function-wide LIFO placement. The opt-in allocation analyzer covers direct allocators, simple aliases/assignments, global parameter/return contracts, owned double-pointer outputs, resolved instance/static method parameter contracts, annotated method returns, direct call-return propagation, equal-provenance `if/else` joins, and same-domain ternary arms; broader expression and branch-sensitive dataflow remain open. Host-`cc` integration covers the supported lowering subset and source-mapped diagnostics. The prototype still fails closed on comptime and C-plus imports, and production `CPlusTranspiler` remains authoritative.
- Phase 6 allocation-analysis verification: targeted JVM tests exercise direct and aliased AST-domain mismatches, positive matching provenance, direct reassignment, global-function parameter checks at call sites, return contracts merged across prototype/definition pairs, owned double-pointer output writes and their call-site treatment, resolved instance/static method parameter checks, annotated method return contracts and source spans, function/method call-return provenance at argument sites, equal and divergent conditional branch joins, matching and divergent ternary arms, assignment/comma expression result provenance, block-scope shadowing/non-leakage, non-propagation from a conditional expression or missing `else`, identifier-exact source spans, and propagation into both prototype and `TranscodedSource` results. Loop and general short-circuit flow parity is deliberately not claimed; the legacy analyzer remains the production path.
