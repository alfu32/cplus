...
| TS-01 | Pinned Tree-sitter C grammar/runtime integration | 2 | Host-specific JNI resources and a six-host CI matrix are configured; Linux x86_64/JDK 21 verified locally, remote matrix pending | Reproducible generation, Java 21-compatible loading/package proof on each supported host, license/provenance, CI parser build. |
...
| MOD-01 | Canonical source/import graph and C/C-plus boundary | 4 | Shared `SourceManager`, deterministic graph ordering, canonical edges, and root-mapped import-cycle diagnostics are implemented; IDE integration and path-namespace breadth remain open | Relative/module/stdlib resolution, canonical identity, cycle diagnostics, C includes unchanged. |
...
- [ ] Integrate ordering with IDE consumers and verify all path namespaces. Existing-path symlink aliases are covered by a host-capability-aware `SourceId` canonicalization test.
...
- [x] Index function-pointer typedefs as callable signatures (result type, parameter names/base types and original declaration spellings, variadic marker, declarator span), excluding identifiers nested inside parameter lists when resolving the typedef name.
- [ ] Complete nested/qualified function-pointer declarator modeling, less common qualified C typedef forms, comptime scopes, and diagnostics for invalid receiver use.
...
- [ ] Port allocation diagnostics and keep the legacy path until parity gates pass.
...
- Phase 2 grammar verification: upstream `tree-sitter-c` v0.24.2 at `b780e47fc780ddc8da13afa35a3f4ed5c157823d`; Tree-sitter CLI 0.25.10 / ABI 15; all 90 upstream+extension corpus cases pass, and generated parser source matches regeneration. Extension cases cover methods, defer, marked/unmarked comptime values, inline comptime expressions, imports, tests, platform-conditioned `@if` chains, `@throws`, `@try`/`@catch`, and comptime identifier interpolation.
...
- Latest verification: legacy-output golden and focused interpolation/AST tests pass with exact generated-C, source-map, and stable-node assertions. The shared legacy/Tree-sitter construct gate passes for eight representative standard-library files and the interpolation fixture. All 90 Tree-sitter corpus cases pass, generated parser source matches regeneration, and `./gradlew test build --no-daemon` passes on Linux x86_64 / JDK 21 after the shadow-walker update. The parser JNI output path and workflow YAML were checked locally; other host-matrix jobs remain unexecuted. Scoped `git diff --check` is clean for compiler, parser, workflow, and architecture-document changes; unrelated staged planning files are preserved.
