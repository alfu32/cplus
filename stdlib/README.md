...
Import `stdlib:/concurrency/thread_pool.cp` for `thread_pool_t` and `thread_task_t`. A pool uses exactly one or two native threads; logical workers are stackless step callbacks that save continuation state in caller-owned context and return `yield`, `wait`, `done`, or `failed(error_code)`. `yield` requeues at the FIFO tail, `wait` parks until another event source calls `wake`, and a wake concurrent with a running callback is latched if that callback then parks. Terminal results remove the task from the active queue. Separate internal condition variables handle worker wakeups and task waiters. This is cooperative scheduling, not arbitrary C-stack suspension: every callback must be bounded and must not wait on I/O, sleeps, or application locks. Callbacks run outside the pool lock, so brief pool API calls are safe. Task/context memory must remain stable through completion. The pool allocates no memory, and `xmem` itself is still not thread-safe. See the [thread-pool specification](../documentation/spec/stdlib/THREAD_POOL.SPEC.md) and run `cpc test stdlib/tests/thread_pool.cp`.
