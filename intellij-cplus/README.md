...
Comptime keywords and identifier splices (for example, `mapper__@name(T)__to__@name(R)`) are highlighted and keyword-led result kinds are offered in completion. Simple object-like `#define generated_name public_name` aliases are suggested as callable names. The plugin does not evaluate comptime expansions; the alias uses ordinary C preprocessor behavior, not a C-plus alias feature.
