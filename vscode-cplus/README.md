...
Open this directory in VS Code and press `F5` to launch an Extension Development Host. Set `cplus.parserCommand` to the C-plus CLI and enable `cplus.parserDiagnostics` to consume `cplus.parse.v1` diagnostics on save. With parser diagnostics enabled, the Outline view also uses the normalized tree for struct fields/methods and top-level functions when the parse matches the current document revision; it falls back to the local index if parsing is unavailable or stale. Set `cplus.compilerCommand` and enable `cplus.compilerDiagnostics` for compile-backed diagnostics. Parser features require a CLI build with the Tree-sitter native runtime for the current host. The local indexer remains available without a language server and understands structs, methods, fields, comptime names, and generated method names.
